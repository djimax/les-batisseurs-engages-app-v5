...
export default function Memberships() {
  ...
  const {
    ...
    isLoading,
    error,
  } = trpc.memberships.listMemberships.useQuery({ memberId });
  ...
  const activeCount = (memberships as any[]).filter(
    (m: any) => m.paymentStatus === "paid"
  ).length;
  const totalAmount = (memberships as any[]).reduce(
    ...
    <div className="space-y-6">
                ...
                <CardContent className="space-y-2">
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      {type.monthlyAmount?.toLocaleString()} €
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Annuel</p>
                    <p className="text-lg font-bold">
                      {type.yearlyAmount?.toLocaleString()} €
                    </p>
                  </div>
                  {type.benefits && (
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Erreur lors du chargement des adhésions
          </AlertDescription>
        </Alert>
      )}
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Badge
                        className={getStatusColor(membership.paymentStatus)}
                      >
