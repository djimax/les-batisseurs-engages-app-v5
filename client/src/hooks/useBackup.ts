...
export function useBackup() {
  const exportData = useCallback(() => {
    try {
      ...
      const documents = JSON.parse(
        localStorage.getItem("offlineDocuments") || "[]"
      );
      const members = JSON.parse(
        localStorage.getItem("offlineMembers") || "[]"
      );
      const categories = JSON.parse(
        localStorage.getItem("offlineCategories") || "[]"
      );
      ...
      reader.onload = e => {
        try {
          ...
          localStorage.setItem(
            "offlineDocuments",
            JSON.stringify(backupData.documents || [])
          );
          localStorage.setItem(
            "offlineMembers",
  ...
  const getBackupSize = useCallback(() => {
    try {
      ...
      const totalSize =
        (documents.length + members.length + categories.length + notes.length) /
        1024;
