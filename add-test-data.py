#!/usr/bin/env python3
import mysql.connector
import os
from dotenv import load_dotenv
from urllib.parse import urlparse

# Load environment variables
load_dotenv('.env.local')

# Get database connection string
database_url = os.getenv('DATABASE_URL')

if not database_url:
    print('ERROR: DATABASE_URL not set in .env.local')
    exit(1)

try:
    # Parse the connection string
    parsed = urlparse(database_url)
    
    host = parsed.hostname
    port = parsed.port or 3306
    user = parsed.username
    password = parsed.password
    db = parsed.path.lstrip('/').split('?')[0]
    
    print(f'Connecting to {host}:{port}/{db}...')
    
    # Connect to database
    connection = mysql.connector.connect(
        host=host,
        user=user,
        password=password,
        database=db,
        port=port,
        use_pure=True
    )
    
    cursor = connection.cursor()
    
    # Check if project already exists
    cursor.execute("SELECT COUNT(*) FROM projects WHERE title = %s", ('Projet Test - Rénovation Bâtiment',))
    if cursor.fetchone()[0] > 0:
        print('✅ Test project already exists')
        cursor.close()
        connection.close()
        exit(0)
    
    # Add the test project with correct column names and enum values
    print('\nAdding test project...')
    
    insert_project = """
    INSERT INTO projects (title, description, startDate, endDate, budget, status, progress, createdBy) 
    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    """
    
    project_data = (
        'Projet Test - Rénovation Bâtiment',
        'Projet de test pour vérifier le fonctionnement du système de gestion de projets',
        '2026-04-15',
        '2026-06-30',
        50000,
        'in-progress',  # Use correct enum value
        25,
        1  # createdBy = admin user
    )
    
    cursor.execute(insert_project, project_data)
    project_id = cursor.lastrowid
    connection.commit()
    
    print(f'✅ Test project created with ID: {project_id}')
    
    # Add test tasks with correct enum values
    print('Adding test tasks...')
    
    insert_task = """
    INSERT INTO tasks (projectId, title, status, priority, createdBy) 
    VALUES (%s, %s, %s, %s, %s)
    """
    
    # Use correct enum values: status in ('todo','in-progress','review','done','cancelled')
    # priority in ('low','medium','high','urgent')
    tasks = [
        ('Planification et devis', 'done', 'high'),
        ('Commande des matériaux', 'in-progress', 'high'),
        ('Préparation du site', 'todo', 'medium'),
        ('Travaux de rénovation', 'todo', 'high'),
        ('Finitions et nettoyage', 'todo', 'medium'),
    ]
    
    for title, status, priority in tasks:
        cursor.execute(insert_task, (project_id, title, status, priority, 1))
    
    connection.commit()
    print(f'✅ {len(tasks)} test tasks created')
    
    cursor.close()
    connection.close()
    
    print('\n✅ All operations completed successfully!')
    print(f'\n📋 Test project details:')
    print(f'   Project ID: {project_id}')
    print(f'   Title: Projet Test - Rénovation Bâtiment')
    print(f'   Budget: 50000 €')
    print(f'   Status: in-progress')
    print(f'   Progress: 25%')
    print(f'\n🎉 You can now view this project in the application at /projects')
    
except Exception as e:
    print(f'ERROR: {e}')
    import traceback
    traceback.print_exc()
    exit(1)
