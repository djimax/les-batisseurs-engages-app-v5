...
async function addTestProject() {
  ...
  try {
    ...
    const description = 'Projet de test pour vérifier le fonctionnement du système de gestion de projets';
    ...
    const [projectResult] = await connection.execute(
      `INSERT INTO projects (name, description, startDate, endDate, budget, status, priority, createdBy, progress) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [projectName, description, startDate, endDate, budget, status, priority, createdBy, 25]
    );
    ...
    // Add some test tasks
    const tasks = [
      { title: 'Planification et devis', status: 'completed', priority: 'Haute' },
      { title: 'Commande des matériaux', status: 'in_progress', priority: 'Haute' },
      { title: 'Préparation du site', status: 'pending', priority: 'Moyenne' },
      { title: 'Travaux de rénovation', status: 'pending', priority: 'Haute' },
      { title: 'Finitions et nettoyage', status: 'pending', priority: 'Moyenne' },
    ];
    ...
    console.log(`📅 Duration: ${startDate.toLocaleDateString('fr-FR')} to ${endDate.toLocaleDateString('fr-FR')}`);
